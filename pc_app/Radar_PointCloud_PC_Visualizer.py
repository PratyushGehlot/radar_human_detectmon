import sys, os, serial, re, numpy as np, time, socket, threading, json
...
# =====================================================
# CONFIG FILE HANDLING (EXTERNAL JSON NEXT TO EXE)
# =====================================================
def get_config_path():
    # When running as EXE
...
# ---- Hard failure with clear message if missing ----
if not os.path.exists(CONFIG_FILE):
    ...
    QtWidgets.QMessageBox.critical(
        ...
        "Please keep config.json in the same folder as the EXE."
    )
    sys.exit(1)
...
# =====================================================
# HARD-CODED APP IDENTITY (AS REQUESTED)
# =====================================================
APP_TITLE = "Privacy Safe Radar based Home Safety Monitoring"
APP_AUTHOR = "Pratyush"
...
# ================= UART =================
SERIAL_PORT = cfg["UART"]["SERIAL_PORT"]
BAUDRATE = cfg["UART"]["BAUDRATE"]

# ================= WIFI =================
WIFI_HOST = cfg["WIFI"]["HOST"]
WIFI_PORT = cfg["WIFI"]["PORT"]

# ================= CALIBRATION =================
STANDING_Z = cfg["CALIBRATION"]["STANDING_Z"]
SITTING_Z = cfg["CALIBRATION"]["SITTING_Z"]
LYING_Z = cfg["CALIBRATION"]["LYING_Z"]

# ================= DETECTION =================
EPS = cfg["DETECTION"]["EPS"]
MIN_SAMPLES = cfg["DETECTION"]["MIN_SAMPLES"]
HUMAN_CONF_THRESHOLD = cfg["DETECTION"]["HUMAN_CONF_THRESHOLD"]
V_MOVE_THRESHOLD = cfg["DETECTION"]["V_MOVE_THRESHOLD"]
FALL_V_THRESHOLD = cfg["DETECTION"]["FALL_V_THRESHOLD"]
...
# ===== TOP-LEFT OVERLAY ON GL VIEW =====
overlay_label = QtWidgets.QLabel(
    f"{APP_TITLE}\nAuthor: {APP_AUTHOR}",
    parent=view
)
...
overlay_label.setAlignment(QtCore.Qt.AlignLeft | QtCore.Qt.AlignTop)
overlay_label.setAttribute(QtCore.Qt.WA_TransparentForMouseEvents)

# Position: top-left inside GL view
overlay_label.move(10, 10)
overlay_label.show()
        ...
        for line in read_lines():
            if "-----PointNum" in line:
                if frame_points:
                    ...
                    add_overlay((0.2, 1.6, GRID_Z + 0.5),f"Human Count: {human_count}",color=(0, 255, 255, 255))
