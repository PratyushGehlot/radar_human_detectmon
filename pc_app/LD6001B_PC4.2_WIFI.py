import sys, serial, re, numpy as np, time, socket, threading
...
def update():
    ...
    global human_boxes, human_labels
    try:
        for line in read_lines():
            if "-----PointNum" in line:
                if frame_points:
                    ...
                    avg_y = np.mean(pts[:,1])
                    avg_z = np.mean(pts[:,2])
                    avg_snr = np.mean(pts[:,3])
                    avg_abs = np.mean(pts[:,4])
                    avg_dpk = np.mean(pts[:,5])
                    avg_v = np.mean(pts[:,6])
                    info_lines = [f"Points:{n}"]
                    for i in range(min(n, 5)):
                        p = pts[i]
                        info_lines.append(
                            f"[{i}] x={p[0]-GRID_X/2:.2f}, y={p[1]-GRID_Y/2:.2f}, "
                            f"z={GRID_Z-p[2]:.2f}, v={p[6]:.2f}, "
                            f"snr={p[3]:.2f}, abs={p[4]:.2f}, dpk={p[5]:.2f}"
                        )
                    info_lines.append(
                        f"Avg: x={avg_x-GRID_X/2:.2f}, y={avg_y-GRID_Y/2:.2f}, "
                        f"z={GRID_Z-avg_z:.2f}, v={avg_v:.2f}, "
                        f"snr={avg_snr:.2f}, abs={avg_abs:.2f}, dpk={avg_dpk:.2f}"
                    )
                    frame_info_label.setText("\n".join(info_lines))
